...
onMounted(async () => {
  initializeTheme();
  adminSidebarCompact.value = window.localStorage.getItem(adminSidebarStorageKey) === 'compact';
...
const navEntries: UiSidebarNavEntry[] = [
  { id: '/', label: 'Inicio', icon: 'home' },
  {
    ...
    icon: 'building',
    children: [
      { id: '/empresas', label: 'Empresas fiscales', icon: 'settings' },
      { id: '/fiscal/onboarding', label: 'Registrar empresa', icon: 'plus' }
    ]
  },
  ...
  { id: '/audit', label: 'Auditoría', icon: 'audit' },
  {
    ...
    icon: 'mail',
    children: [
      { id: '/notifications/sender-aliases', label: 'Alias de remitente' },
...
const activeNavId = computed(() => {
  for (const entry of navEntries) {
    ...
    } else if (isActivePath(entry.id)) return entry.id;
  }
  return '/';
});
...
const breadcrumbs = computed(() => {
  ...
  for (const entry of navEntries) {
    ...
    if (isGroup(entry)) {
      const activeChild = entry.children.find((item) => isActivePath(item.id));

      if (activeChild) {
        return [{ label: activeChild.label, to: activeChild.id }];
      }
    }
  }
...
function adminIconComponent(icon?: string): Component {
  ...
  return icons[icon || 'home'] ?? Settings;
}
</script>
  ...
  <div v-else class="sf-app-background min-h-screen">
    <aside v-if="!companyDetailActive" class="fixed inset-y-0 left-0 hidden flex-col border-r border-slate-200 bg-white text-[15.5px] transition-[width] lg:flex" :class="adminSidebarCompact ? 'w-20' : 'w-72'">
      ...
      <div class="flex-1 overflow-y-auto py-5" :class="adminSidebarCompact ? 'px-3' : 'px-4'">
        <UiHierarchicalSidebarNav :entries="navEntries" :active-id="activeNavId" :compact="adminSidebarCompact" @select="selectAdminNav" @request-expand="adminSidebarCompact = false">
          <template #icon="{ entry }"><component :is="adminIconComponent(entry.icon)" class="h-[22px] w-[22px]" /></template>
