...
<template>
  <div ref="menuRef" class="relative">
    <button
      type="button"
      class="flex w-full items-center gap-3 rounded-xl bg-white p-2 text-left text-slate-950 transition hover:bg-slate-50 focus:outline focus:outline-2 focus:outline-offset-2 focus:outline-sky-500 dark:bg-surface-raised dark:text-text dark:hover:bg-surface-strong"
    ...
    <div
      v-if="open"
      class="absolute bottom-full left-0 z-30 mb-3 w-80 origin-bottom-left rounded-2xl border border-slate-200 bg-white p-4 text-slate-900 shadow-xl shadow-slate-950/15 dark:border-line dark:bg-surface dark:text-text dark:shadow-black/30"
      role="menu"
    >
      <div class="px-1 pb-4">
        <p class="truncate text-sm font-semibold text-slate-950 dark:text-text">{{ name ?? 'Stelfaro' }}</p>
        <p class="mt-1 truncate text-sm text-slate-500 dark:text-muted">{{ email ?? 'Sesion activa' }}</p>
        <div class="mt-4 flex items-start gap-3 rounded-xl px-3 py-3" :class="statusClasses.bg">
          ...
          <span class="min-w-0">
            ...
            <span class="mt-0.5 block text-xs leading-5 text-slate-600 dark:text-muted">{{ status.description }}</span>
          </span>
        </div>
      </div>

      <div class="space-y-1 py-2">
        <RouterLink to="/" class="flex items-center gap-4 rounded-lg px-3 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50 hover:text-slate-950 dark:text-muted dark:hover:bg-surface-muted dark:hover:text-text" role="menuitem" @click="close">
        ...
        <RouterLink to="/empresas" class="flex items-center gap-4 rounded-lg px-3 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50 hover:text-slate-950 dark:text-muted dark:hover:bg-surface-muted dark:hover:text-text" role="menuitem" @click="close">
          <svg class="h-5 w-5 text-slate-500 dark:text-soft" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true">
        ...
        <RouterLink to="/fiscal/onboarding" class="flex items-center gap-4 rounded-lg px-3 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50 hover:text-slate-950 dark:text-muted dark:hover:bg-surface-muted dark:hover:text-text" role="menuitem" @click="close">
          <svg class="h-5 w-5 text-slate-500 dark:text-soft" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true">
      ...
      <div class="space-y-1 border-t border-slate-200 py-2 dark:border-line">
        <button type="button" class="flex w-full items-center justify-between gap-4 rounded-lg px-3 py-3 text-left text-sm font-semibold text-slate-700 hover:bg-slate-50 hover:text-slate-950 dark:text-muted dark:hover:bg-surface-muted dark:hover:text-text" role="menuitem" @click="toggleTheme">
        ...
        <button type="button" class="flex w-full items-center gap-4 rounded-lg px-3 py-3 text-left text-sm font-semibold text-slate-700 hover:bg-rose-50 hover:text-rose-700 dark:text-muted dark:hover:bg-danger-soft dark:hover:text-danger" role="menuitem" @click="logout">
          <svg class="h-5 w-5 text-slate-500 dark:text-soft" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true">
