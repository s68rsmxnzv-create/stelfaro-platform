...
export type PlatformPaymentBreakdown = { cash: number; card: number; transfer: number; other: number };
export type PlatformSalesReport = { summary: { transactions: number; net: number; tax: number; total: number; receivable: number; cost: number; margin: number; payments: PlatformPaymentBreakdown & { recorded: number; unclassified: number } }; data: Array<{ id: number; date: string|null; source_type: string; source_id: string; source_number: string|null; document_type: string|null; operation_kind: string; customer_name: string|null; payment_status: string; outstanding_amount: number; payment_methods: PlatformPaymentBreakdown; collected: number; net: number; tax: number; total: number }>; meta: { current_page: number; last_page: number; total: number } };
