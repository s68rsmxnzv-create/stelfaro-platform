...
import { PlatformClient, type PlatformFiscalScopeResponse, type PlatformTenantRequest, type PlatformTenantRequestStatus, type PlatformTenantRequestType } from '@stelfaro/api-client';
...
const form = reactive({ type: 'user_access' as PlatformTenantRequestType, subject: '', description: '', action: 'create', name: '', email: '', phone: '', role: 'billing_user', address: '', code: '', establishmentType: 'branch', department: '', municipality: '', district: '', branchId: '', pointType: 'terminal' });
...
const canSubmit = computed(() => {
  ...
  if (form.type === 'user_access' && form.action === 'create') return Boolean(form.name.trim() && form.email.trim() && form.role);
  if (form.type === 'branch') return Boolean(form.name.trim() && form.address.trim() && form.department.trim() && form.municipality.trim());
  if (form.type === 'point_of_sale') return Boolean(form.name.trim() && Number(form.branchId) > 0 && form.pointType);
  return true;
});
...
onMounted(async () => {
  ...
  await load();
  try { fiscalScope.value = await client.tenantFiscalScope(tenantId.value); } catch { fiscalScope.value = null; }
...
function reset(): void { Object.assign(form, { type: 'user_access', subject: 'Crear o modificar usuario', description: '', action: 'create', name: '', email: '', phone: '', role: 'billing_user', address: '', code: '', establishmentType: 'branch', department: '', municipality: '', district: '', branchId: '', pointType: 'terminal' }); }
...
async function submit(): Promise<void> {
  ...
  try {
    ...
    if (form.type === 'branch') Object.assign(payload, { name: form.name.trim(), establishment_type: form.establishmentType, address: form.address.trim(), department: form.department.trim(), municipality: form.municipality.trim(), district: form.district.trim() || null, phone: form.phone.trim() || null, email: form.email.trim() || null });
    if (form.type === 'point_of_sale') { const branch = fiscalScope.value?.sucursales.find(item => item.id === Number(form.branchId)); Object.assign(payload, { name: form.name.trim(), branch_id: Number(form.branchId), branch_name: branch?.nombre ?? null, point_type: form.pointType }); }
...
<template>
  ...
  <UiModalShell :open="modalOpen" title="Nueva solicitud" description="La solicitud llegará al panel administrativo de Stelfaro." max-width="max-w-2xl" @close="modalOpen = false">
    <div class="space-y-4"><UiSelect v-model="form.type" label="Tipo de solicitud" :options="typeOptions" /><UiInput v-model="form.subject" label="Asunto" />
      ...
      <template v-else-if="form.type === 'branch'"><div class="grid gap-4 sm:grid-cols-2"><UiSelect v-model="form.establishmentType" label="Tipo de establecimiento" :options="establishmentOptions" /><UiInput v-model="form.name" label="Nombre" /><UiInput v-model="form.address" class="sm:col-span-2" label="Dirección" /><UiInput v-model="form.department" label="Departamento" /><UiInput v-model="form.municipality" label="Municipio" /><UiInput v-model="form.district" label="Distrito" /><UiPhoneInput v-model="form.phone" label="Teléfono" /><UiEmailInput v-model="form.email" label="Correo" /><div class="rounded-xl border border-line bg-surface-muted p-4 text-sm text-muted sm:col-span-2">El código del establecimiento y su primer punto de venta serán asignados durante la revisión administrativa.</div></div></template>
      <template v-else-if="form.type === 'point_of_sale'"><div class="grid gap-4 sm:grid-cols-2"><UiSelect v-model="form.branchId" label="Sucursal" :options="branchOptions" /><UiInput v-model="form.name" label="Nombre del punto de venta" /><UiSelect v-model="form.pointType" label="Tipo" :options="pointTypeOptions" /><div class="rounded-xl border border-line bg-surface-muted p-4 text-sm text-muted sm:col-span-2">El código, normalmente P001 para el primer punto, será confirmado por administración.</div></div></template>
