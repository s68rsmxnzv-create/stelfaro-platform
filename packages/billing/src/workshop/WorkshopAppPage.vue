...
async function printReception(order: WorkshopOrder, photoUrl?: string) { if (receptionPrinting.value) return; receptionPrinting.value = true; try { const [ticketSettingsResult, accessResult] = await Promise.all([platform.workshopTicketSettings(props.tenantId), order.device_access ? Promise.resolve({ data: order.device_access }) : platform.workshopDeviceAccess(props.tenantId, order.id)]); const ticketSettings: WorkshopTicketSettings = ticketSettingsResult.data; const result = await sendSilentPrint(await workshopReceptionTicket(order, props.company, photoUrl, ticketSettings, accessResult.data)); pushFloatingToast(result === 'printed' ? { title: 'Recepción impresa', message: `${order.ticket} fue enviada con ${ticketSettings.receipt_copies} ${ticketSettings.receipt_copies === 1 ? 'copia' : 'copias'}${ticketSettings.print_equipment_label ? ' y etiqueta QR' : ''}.`, variant: 'success' } : { title: 'Recepción lista', message: 'Activa la impresión silenciosa y selecciona una impresora para imprimirla.', variant: 'info' }); } catch (reason) { pushFloatingToast({ title: 'No se pudo imprimir', message: reason instanceof Error ? reason.message : 'Revisa la conexión con la impresora.', variant: 'error' }); } finally { receptionPrinting.value = false; } }
