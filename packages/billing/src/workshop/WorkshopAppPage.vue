...
<template>
  <section class="mx-auto max-w-7xl px-5 py-6">
    ...
    <WorkshopReceptionComplete v-else-if="view === 'reception' && completed" :order="completed.order" :orders="completed.orders" :photos="workshop.photos.value" :photo-loading="workshop.photoLoading.value" :photo-url="completed.url" :expires-at="completed.expires_at" :printing="receptionPrinting" @refresh-photos="workshop.loadPhotos(completed.order.id)" @print="printReception(completed.order)" @add-another="addAnotherEquipment" @finish="finishReception" />
