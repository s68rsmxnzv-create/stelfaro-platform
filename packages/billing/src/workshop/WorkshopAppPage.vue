...
async function create(payload: WorkshopOrderPayload) { const order = await workshop.createOrder(payload); const orders=(await platform.workshopReception(props.tenantId, order.reception.id)).data.orders; try { const session = await workshop.createPhotoSession(order.id); completed.value = { order, orders, ...session }; } catch { completed.value = { order, orders, url: '', expires_at: '' }; } continuation.value=null; await workshop.loadPhotos(order.id); }
function addAnotherEquipment() { if (!completed.value) return; continuation.value={ receptionId:completed.value.order.reception.id, customer:completed.value.order.customer, branchId:completed.value.order.branch?.id ?? null }; completed.value=null; }
function finishReception() { completed.value=null; continuation.value=null; }
...
async function printReception(order: WorkshopOrder) { if (receptionPrinting.value) return; receptionPrinting.value = true; try { const printerSettings = loadPrinterSettings(); const receptionPromise=platform.workshopReception(props.tenantId, order.reception.id); const ticketSettingsPromise = platform.workshopTicketSettings(props.tenantId); const logoPromise = printerSettings.showLogo && props.company?.logo_url ? core.companyThermalLogo(props.company.id, printerSettings.paperWidth === '58' ? 240 : 320) : Promise.resolve({ logo: null }); const [receptionResult,ticketSettingsResult,logoResult]=await Promise.all([receptionPromise,ticketSettingsPromise,logoPromise]); const orders=receptionResult.data.orders; const accessEntries=await Promise.all(orders.map(async item => [item.id,(item.device_access || (await platform.workshopDeviceAccess(props.tenantId,item.id)).data)] as const)); const accesses=Object.fromEntries(accessEntries); if (printerSettings.showLogo && props.company?.logo_url && !logoResult.logo) throw new Error('No fue posible preparar el logo para la impresora.'); const ticketSettings: WorkshopTicketSettings = ticketSettingsResult.data; const result = await sendSilentPrint(await workshopReceptionTicket(orders, props.company, ticketSettings, accesses, logoResult.logo)); pushFloatingToast(result === 'printed' ? { title: 'Recepción impresa', message: `${order.ticket} fue enviada con ${orders.length} ${orders.length === 1 ? 'equipo' : 'equipos'} y ${ticketSettings.receipt_copies} ${ticketSettings.receipt_copies === 1 ? 'copia' : 'copias'}.`, variant: 'success' } : { title: 'Recepción lista', message: 'Activa la impresión silenciosa y selecciona una impresora para imprimirla.', variant: 'info' }); } catch (reason) { pushFloatingToast({ title: 'No se pudo imprimir', message: reason instanceof Error ? reason.message : 'Revisa la conexión con la impresora.', variant: 'error' }); } finally { receptionPrinting.value = false; } }
...
async function recordPayment(id: number, payload: Parameters<typeof workshop.recordPayment>[1]) { const updated = await workshop.recordPayment(id, payload); if (completed.value) { completed.value.orders = completed.value.orders.map(order => order.id === id ? updated : order); if (completed.value.order.id === id) completed.value.order = updated; } paymentOrderId.value = null; pushFloatingToast({ title: completed.value && !updated.financial.closed_at ? 'Anticipo registrado' : 'Pago registrado', message: updated.balance > 0 ? `Saldo pendiente: $${updated.balance.toFixed(2)}.` : 'La orden quedó saldada.', variant: 'success' }); }
...
<template>
  <section class="mx-auto max-w-7xl px-5 py-6">
    ...
    <WorkshopReceptionComplete v-else-if="view === 'reception' && completed" :order="completed.order" :orders="completed.orders" :photos="workshop.photos.value" :photo-loading="workshop.photoLoading.value" :photo-url="completed.url" :expires-at="completed.expires_at" :printing="receptionPrinting" @refresh-photos="workshop.loadPhotos(completed.order.id)" @print="printReception(completed.order)" @advance="paymentOrderId = $event.id" @add-another="addAnotherEquipment" @finish="finishReception" />
    <WorkshopReceptionForm v-else-if="view === 'reception'" :customers="workshop.customers.value" :branches="company?.sucursales || []" :catalogs="catalogs" :customer-loading="workshop.customerLoading.value" :continuation="continuation" :on-create-customer="workshop.createCustomer" :on-save="create" @search="workshop.searchCustomers" />
    <template v-else>
