...
import { CoreDteClient, PlatformClient, type BillingEmpresa, type WorkshopOrder, type WorkshopOrderPayload, type WorkshopTicketSettings } from '@stelfaro/api-client';
...
async function printReception(order: WorkshopOrder, photoUrl?: string) { if (receptionPrinting.value) return; receptionPrinting.value = true; try { const ticketSettingsPromise = platform.workshopTicketSettings(props.tenantId); const accessPromise = order.device_access ? Promise.resolve({ data: order.device_access }) : platform.workshopDeviceAccess(props.tenantId, order.id); const photoSessionPromise = photoUrl ? Promise.resolve({ url: photoUrl }) : workshop.createPhotoSession(order.id).catch(() => ({ url: '' })); const [ticketSettingsResult, accessResult, photoSessionResult] = await Promise.all([ticketSettingsPromise, accessPromise, photoSessionPromise]); const ticketSettings: WorkshopTicketSettings = ticketSettingsResult.data; const result = await sendSilentPrint(await workshopReceptionTicket(order, props.company, photoSessionResult.url, ticketSettings, accessResult.data)); pushFloatingToast(result === 'printed' ? { title: 'Recepción impresa', message: `${order.ticket} fue enviada con ${ticketSettings.receipt_copies} ${ticketSettings.receipt_copies === 1 ? 'copia' : 'copias'}${ticketSettings.print_equipment_label ? ' y etiqueta QR' : ''}.`, variant: 'success' } : { title: 'Recepción lista', message: 'Activa la impresión silenciosa y selecciona una impresora para imprimirla.', variant: 'info' }); } catch (reason) { pushFloatingToast({ title: 'No se pudo imprimir', message: reason instanceof Error ? reason.message : 'Revisa la conexión con la impresora.', variant: 'error' }); } finally { receptionPrinting.value = false; } }
...
<template>
  <section class="mx-auto max-w-7xl px-5 py-6">
    ...
    <WorkshopReceptionComplete v-else-if="view === 'reception' && completed" :order="completed.order" :photos="workshop.photos.value" :photo-loading="workshop.photoLoading.value" :photo-url="completed.url" :expires-at="completed.expires_at" :printing="receptionPrinting" @refresh-photos="workshop.loadPhotos(completed.order.id)" @print="printReception(completed.order, completed.url)" @reset="completed = null" />
