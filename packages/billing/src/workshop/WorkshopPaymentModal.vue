...
<template>
  <UiModalShell :open="Boolean(order)" :title="isAdvance ? 'Registrar anticipo' : 'Registrar pago'" :description="order ? `${order.ticket} · ${order.customer.name}` : null" max-width="max-w-lg" @close="$emit('close')">
    <div v-if="order" class="space-y-4">
      ...
      <div class="flex justify-end gap-2 border-t border-line pt-4"><UiButton variant="secondary" @click="$emit('close')">Volver</UiButton><UiButton variant="success" :disabled="saving || Number(form.amount) <= 0 || Number(form.amount) > order.balance" @click="submit"><HandCoins class="h-4 w-4" />{{ isAdvance ? 'Registrar anticipo' : 'Registrar pago' }}</UiButton></div>
    </div>
  </UiModalShell>
</template>
