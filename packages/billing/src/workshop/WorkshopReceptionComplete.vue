...
const whatsappUrl = computed(() => {
  ...
  const lines = [
    ...
    ...(props.order.device.color ? [`• Color: ${props.order.device.color}`] : []),
    `• ${identifier}`,
    `• Encendido: ${powerLabels[props.order.device.power_status] || props.order.device.power_status}`,
    ...(props.order.device.is_locked ? [`• Acceso para revisión: ${props.order.device.has_access_secret ? 'Sí, fue proporcionado' : 'No fue proporcionado'}`] : []),
    '',
