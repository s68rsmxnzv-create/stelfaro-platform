...
const emit = defineEmits<{ close: []; settle: [id: number, payload: { action: 'deliver_close' | 'cancel_close'; final_total?: number; retained_amount?: number; amount_received?: number; method?: 'cash' | 'card' | 'transfer' | 'other'; reference?: string | null; notes?: string | null; document_choice?: 'work_order'|'dte'; dte_type?: '01'|'03' }] }>();
const form = reactive({ finalTotal: '', retainedAmount: '0', amountReceived: '', method: 'cash' as 'cash'|'card'|'transfer'|'other', reference: '', notes: '', documentChoice: 'work_order' as 'work_order'|'dte', dteType: '01' as '01'|'03' });
...
watch(() => props.order, (order) => {
  ...
  form.retainedAmount = '0'; form.amountReceived = Math.max(0, Number(form.finalTotal) - order.paid_total).toFixed(2); form.method = 'cash'; form.reference = ''; form.notes = ''; form.documentChoice = 'work_order'; form.dteType = '01';
}, { immediate: true });
function submit() {
  ...
  <UiModalShell :open="Boolean(order)" :title="isCancellation ? 'Resolver anticipo y cerrar' : 'Entregar y cerrar orden'" :description="order ? `${order.ticket} · ${order.customer.name}` : null" max-width="max-w-xl" :close-on-backdrop="!saving" @close="$emit('close')">
    <div v-if="order" class="space-y-5">
      ...
      <div class="grid grid-cols-2 gap-3 rounded-lg border border-line bg-surface-muted p-4 text-sm">
        ...
        <div><p class="text-muted">{{ isCancellation ? 'A devolver' : 'Quedará por cobrar' }}</p><p class="mt-1 text-lg font-bold" :class="isCancellation || remainingAfterDelivery > 0 ? 'text-warning' : 'text-success'">{{ money(isCancellation ? amountToRefund : remainingAfterDelivery) }}</p></div>
      </div>

      <div v-if="isCancellation && amountToRefund > 0" class="grid gap-3 sm:grid-cols-2"><UiSelect v-model="form.method" label="Forma de devolución" :options="methods" /><UiInput v-model="form.reference" label="Referencia" placeholder="Opcional" /></div>
      ...
      <div class="flex justify-end gap-2 border-t border-line pt-4"><UiButton variant="secondary" :disabled="saving" @click="$emit('close')">Volver</UiButton><UiButton :variant="isCancellation ? 'secondary' : 'success'" :disabled="saving || (isCancellation && retained > order.paid_total) || (!isCancellation && amountReceived > amountToCollect)" @click="submit"><CircleDollarSign class="h-4 w-4" />{{ isCancellation ? 'Registrar y cerrar' : amountReceived <= 0 ? 'Entregar con saldo pendiente' : remainingAfterDelivery > 0 ? 'Registrar abono y entregar' : 'Cobrar, entregar y cerrar' }}</UiButton></div>
    </div>
  </UiModalShell>
</template>
