      ...
      <div v-if="!isCancellation" class="md:hidden">
        ...
        <section v-else class="space-y-4">
          ...
          <div class="sticky bottom-0 -mx-4 grid grid-cols-[auto_1fr] gap-2 border-t border-line bg-surface/95 px-4 pb-[max(0.25rem,env(safe-area-inset-bottom))] pt-3 backdrop-blur">
            ...
            <UiButton class="justify-center" variant="success" :disabled="saving" @click="submit"><CircleDollarSign class="h-4 w-4" />{{ amountToCollect <= 0 ? 'Entregar y cerrar' : amountReceived <= 0 ? 'Entregar con saldo' : remainingAfterDelivery > 0 ? 'Abonar y entregar' : 'Cobrar y cerrar' }}</UiButton>
          </div>
        </section>
      </div>
