...
import { Camera, ChevronLeft, ChevronRight, HandCoins, MessageCircle, Microscope, PackageCheck, Printer } from 'lucide-vue-next';
...
const emit = defineEmits<{ search: [params: { q?: string; status?: string; priority?: string; date_from?: string; date_to?: string; page?: number; per_page?: number }]; select: [order: WorkshopOrder]; addPhotos: [order: WorkshopOrder]; diagnose: [order: WorkshopOrder]; settle: [order: WorkshopOrder] }>();
const query = ref('');
const status = ref('');
const priority = ref('');
const dateFrom = ref('');
  ...
  <div class="space-y-4">
    <UiCard class="overflow-hidden">
      ...
      <div class="overflow-x-auto">
        <table class="w-full min-w-[920px] text-left text-sm"><thead class="bg-surface-muted text-muted"><tr><th class="px-4 py-3">Orden</th><th class="px-4 py-3">Cliente y equipo</th><th class="px-4 py-3">Falla reportada</th><th class="px-4 py-3">Estado</th><th class="px-4 py-3">Saldo</th><th class="px-4 py-3">Antigüedad</th><th class="px-4 py-3 text-right">Acciones</th></tr></thead><tbody class="divide-y divide-line"><tr v-for="order in orders" :key="order.id" class="cursor-pointer text-text transition hover:bg-primary-soft/30" tabindex="0" @click="$emit('select', order)" @keydown.enter="$emit('select', order)"><td class="px-4 py-4"><strong class="text-primary">{{ order.ticket }}</strong><p class="mt-1 text-xs capitalize text-muted">{{ order.priority }}</p></td><td class="px-4 py-4"><strong>{{ order.customer.name }}</strong><p class="mt-1 text-muted">{{ order.device.brand }} {{ order.device.model }}</p><p class="text-xs text-muted">{{ order.device.imei || order.device.serial_number || 'Sin identificador' }}</p></td><td class="max-w-xs px-4 py-4"><p class="line-clamp-2">{{ order.reported_fault }}</p></td><td class="px-4 py-4"><UiStatusBadge :tone="order.status === 'ready' ? 'success' : order.status === 'cancelled' ? 'danger' : 'neutral'">{{ labels[order.status] || order.status }}</UiStatusBadge><p v-if="order.financial.status === 'settled'" class="mt-1 text-xs text-success">Orden cerrada</p><p v-else-if="order.status === 'cancelled' && order.paid_total > 0" class="mt-1 text-xs font-medium text-warning">Anticipo por resolver</p></td><td class="px-4 py-4"><template v-if="order.status === 'cancelled' && order.financial.status !== 'settled'"><p class="font-semibold text-warning">${{ order.paid_total.toFixed(2) }}</p><p class="text-xs text-muted">por resolver</p></template><span v-else class="font-semibold">${{ order.balance.toFixed(2) }}</span></td><td class="px-4 py-4 text-muted">{{ age(order.received_at) }}</td><td class="px-4 py-4" @click.stop @keydown.stop><UiActionDropdown :label="`Acciones de ${order.ticket}`"><UiActionMenuItem v-if="['received','diagnosing','awaiting_approval','approved','repairing'].includes(order.status)" @select="$emit('diagnose', order)"><template #icon><Microscope class="h-4 w-4" /></template>{{ workActionLabel(order.status) }}</UiActionMenuItem><UiActionMenuItem v-if="order.status === 'ready' && order.financial.status !== 'settled'" @select="$emit('settle', order)"><template #icon><PackageCheck class="h-4 w-4" /></template>Entregar y cerrar</UiActionMenuItem><UiActionMenuItem v-if="order.status === 'cancelled' && order.financial.status !== 'settled' && order.paid_total > 0" @select="$emit('settle', order)"><template #icon><HandCoins class="h-4 w-4" /></template>Resolver anticipo</UiActionMenuItem><UiActionMenuItem :disabled="!order.customer.phone" @select="sendWhatsApp(order)"><template #icon><MessageCircle class="h-4 w-4" /></template>Reenviar por WhatsApp</UiActionMenuItem><UiActionMenuItem disabled><template #icon><Printer class="h-4 w-4" /></template>Reimprimir ticket · Próximamente</UiActionMenuItem><UiActionMenuItem @select="$emit('addPhotos', order)"><template #icon><Camera class="h-4 w-4" /></template>{{ order.photo_count ? 'Agregar fotos' : 'Agregar primera foto' }}</UiActionMenuItem></UiActionDropdown></td></tr><tr v-if="!orders.length"><td colspan="7" class="px-4 py-12 text-center text-muted">No encontramos órdenes con estos filtros.</td></tr></tbody></table>
      </div>
