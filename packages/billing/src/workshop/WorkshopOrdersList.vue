...
const emit = defineEmits<{ search: [params: { q?: string; status?: string; priority?: string; date_from?: string; date_to?: string; page?: number; per_page?: number }]; select: [order: WorkshopOrder]; addPhotos: [order: WorkshopOrder]; diagnose: [order: WorkshopOrder]; settle: [order: WorkshopOrder]; invoice: [order: WorkshopOrder] }>();
const query = ref('');
const status = ref('');
const priority = ref('');
const dateFrom = ref('');
