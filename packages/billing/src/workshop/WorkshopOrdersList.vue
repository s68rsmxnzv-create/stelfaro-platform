...
const emit = defineEmits<{ search: [params: { q?: string; status?: string; priority?: string; payment_status?: string; date_from?: string; date_to?: string; page?: number; per_page?: number }]; select: [order: WorkshopOrder]; addPhotos: [order: WorkshopOrder]; diagnose: [order: WorkshopOrder]; cancel: [order: WorkshopOrder]; settle: [order: WorkshopOrder]; invoice: [order: WorkshopOrder]; pay: [order: WorkshopOrder]; print: [order: WorkshopOrder]; printReception: [order: WorkshopOrder] }>();
const query = ref('');
const status = ref('');
const priority = ref('');
