...
import { Camera, ChevronDown, ChevronLeft, ChevronRight, ClipboardCheck, Filter, HandCoins, MessageCircle, Microscope, Printer, QrCode, RotateCcw } from 'lucide-vue-next';
        ...
        <article v-for="order in orders" :key="order.id" class="rounded-2xl border border-line bg-surface shadow-sm">
          ...
          <div class="flex items-center gap-2 border-t border-line px-3 py-2.5" @click.stop>
            ...
            <UiActionDropdown :label="`Acciones de ${order.ticket} ${order.reception.equipment_label}`">
              ...
              <UiActionMenuItem v-if="order.status !== 'cancelled'" @select="$emit('printLabel', order)"><template #icon><QrCode class="h-4 w-4" /></template>Imprimir etiqueta del equipo</UiActionMenuItem>
