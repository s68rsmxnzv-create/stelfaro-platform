...
<template>
  <div class="space-y-5">
    ...
    <div class="grid gap-3 rounded-lg border border-line bg-surface-muted p-4 sm:grid-cols-2"><label class="flex cursor-pointer items-center gap-3 text-sm text-text"><input v-model="settings.showLogo" type="checkbox" class="h-4 w-4 accent-primary"><span><strong class="block">Imprimir logo</strong><small class="text-muted">Usa y rasteriza el logo guardado en el perfil de la empresa.</small></span></label><label class="flex cursor-pointer items-center gap-3 text-sm text-text"><input v-model="settings.showIssuerDetails" type="checkbox" class="h-4 w-4 accent-primary"><span><strong class="block">Imprimir información de la empresa</strong><small class="text-muted">Nombre comercial, actividad, NIT y NRC.</small></span></label></div>
