...
<template>
  <div class="space-y-5">
    <div class="flex flex-wrap items-start justify-between gap-4"><div><div class="flex items-center gap-2 text-primary"><Printer class="h-5 w-5" /><h3 class="font-semibold text-text">Impresora de esta terminal</h3></div><p class="mt-2 max-w-2xl text-sm text-muted">Estas preferencias se guardan únicamente en este navegador. No afectan otras computadoras ni usuarios.</p></div><div class="grid gap-3"><label class="flex cursor-pointer items-center gap-3 rounded-lg border border-line bg-surface-muted px-4 py-3"><input v-model="settings.enabled" type="checkbox" class="h-4 w-4 accent-primary"><span class="text-sm font-semibold text-text">Impresión silenciosa activa</span></label><label v-if="settings.enabled" class="flex cursor-pointer items-start gap-3 rounded-lg border border-line bg-surface-muted px-4 py-3"><input v-model="settings.autoPrintAfterIssue" type="checkbox" class="mt-0.5 h-4 w-4 accent-primary"><span class="text-sm text-text"><strong class="block">Impresión automática después de emitir</strong><small class="mt-1 block text-muted">Imprime al recibir Hacienda. Si el cliente tiene correo, preguntaremos antes.</small></span></label></div></div>

    <div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_220px_auto]"><UiInput v-model="settings.agentUrl" label="Dirección del agente" placeholder="http://localhost:8711" /><UiSelect v-model="settings.printer" label="Impresora predeterminada" :options="printers.length ? printers.map(printer => ({ value: printer.name, label: printer.name })) : settings.printer ? [{ value: settings.printer, label: settings.printer }] : []" placeholder="Buscar impresoras" /><div class="flex items-end"><UiButton class="w-full" variant="secondary" :disabled="checking" @click="checkAgent(true)"><RefreshCw class="h-4 w-4" :class="checking ? 'animate-spin' : ''" />Buscar</UiButton></div></div>

    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3"><UiSelect v-model="settings.paperWidth" label="Ancho de papel" :options="[{value:'58',label:'58 mm'},{value:'80',label:'80 mm'}]" /><UiInput v-model.number="settings.cutLines" label="Líneas antes del corte" type="number" min="1" max="12" /><label class="flex cursor-pointer items-center gap-3 rounded-lg border border-line bg-surface-muted px-4 py-3 text-sm text-text lg:self-end"><input v-model="settings.openDrawer" type="checkbox" class="h-4 w-4 accent-primary"><span><strong class="block">Abrir gaveta al cobrar</strong><small class="text-muted">Envía el pulso mediante la impresora seleccionada.</small></span></label></div>
