  ...
  <div class="space-y-5">
    ...
    <div class="grid items-start gap-5 xl:grid-cols-[minmax(300px,0.72fr)_minmax(520px,1.28fr)]">
      <div class="grid gap-3 rounded-lg border border-line bg-surface-muted p-4 sm:p-5 xl:sticky xl:top-4">
        ...
        <label v-if="previewVariant === 'dte'" class="flex cursor-pointer items-start gap-3 rounded-lg border border-line bg-surface p-3 text-sm text-text"><input v-model="settings.qrEnabled" type="checkbox" class="mt-1 h-4 w-4 accent-primary"><span><strong class="block">Imprimir QR fiscal</strong><small class="text-muted">Incluye el acceso a la consulta pública del DTE.</small></span></label>
        <div v-if="previewVariant === 'dte' || workshopSettings.print_equipment_label" class="rounded-lg border border-line bg-surface p-3"><UiInput v-model.number="settings.qrWidth" :label="previewVariant === 'dte' ? 'Tamaño del QR fiscal' : 'Tamaño del QR de la etiqueta'" type="number" min="120" max="420" step="1" suffix="px" /><p class="mt-1 text-xs text-muted">Valor permitido: 120 a 420.</p></div>
      </div>
