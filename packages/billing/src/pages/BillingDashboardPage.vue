...
<template>
  <div class="space-y-5">
    ...
    <section v-if="workshopEnabled" class="grid gap-5 lg:grid-cols-[minmax(0,1.6fr)_minmax(280px,0.8fr)]">
      ...
      <div class="space-y-5">
        <UiCard><div class="flex items-center gap-2"><CalendarDays class="h-5 w-5 text-primary" /><h3 class="font-semibold text-text">Este mes</h3></div><p class="mt-4 text-3xl font-bold text-text">{{ commercial ? money(commercial.sales_month) : '—' }}</p><p class="mt-1 text-sm text-muted">Neto {{ commercial ? money(commercial.sales_net_month) : '—' }} · IVA {{ commercial ? money(commercial.sales_tax_month) : '—' }}</p><div v-if="workshop" class="mt-5 space-y-3"><div v-for="item in activeBreakdown" :key="item.label"><div class="mb-1 flex justify-between text-xs"><span class="text-muted">{{ item.label }}</span><strong class="text-text">{{ item.value }}</strong></div><div class="h-1.5 overflow-hidden rounded-full bg-surface-muted"><div class="h-full rounded-full" :class="item.tone" :style="{ width: `${Math.max(item.value ? 8 : 0, item.value / maxBreakdown * 100)}%` }" /></div></div></div></UiCard>
    ...
    <section v-else class="grid gap-5 lg:grid-cols-2">
      <UiCard><div class="flex items-center gap-2"><CircleDollarSign class="h-5 w-5 text-primary" /><h3 class="font-semibold text-text">Resumen comercial</h3></div><div class="mt-4 divide-y divide-line text-sm"><div class="flex items-center justify-between py-3"><span class="text-muted">Venta bruta de hoy</span><strong class="text-text">{{ commercial ? money(commercial.sales_today) : '—' }}</strong></div><div class="flex items-center justify-between py-3"><span class="text-muted">Venta neta de hoy</span><strong class="text-text">{{ commercial ? money(commercial.sales_net_today) : '—' }}</strong></div><div class="flex items-center justify-between py-3"><span class="text-muted">IVA de hoy</span><strong class="text-text">{{ commercial ? money(commercial.sales_tax_today) : '—' }}</strong></div></div></UiCard>
