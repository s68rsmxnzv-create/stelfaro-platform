...
import TaxEstimateCard from '../components/TaxEstimateCard.vue';
...
const showSkeleton = computed(() => loading.value && !commercial.value && !fiscal.value && !workshop.value);
const fiscalTone = computed<'neutral'|'success'|'warning'>(() => !fiscal.value ? 'neutral' : fiscal.value.totals.rejected ? 'warning' : 'success');
const fiscalLabel = computed(() => !fiscal.value ? 'Sin datos' : fiscal.value.totals.rejected ? 'Hay rechazos' : 'Sin rechazos');
const hasAttention = computed(() => Boolean(quotesPending.value || workshop.value?.orders.urgent || workshop.value?.orders.awaiting_approval));
...
<template>
  <div class="space-y-5">
    <div class="space-y-5 md:hidden">
      ...
      <Teleport to="body">
        <div
          ...
          class="fixed inset-0 z-[1000] flex items-end justify-center bg-black/55 backdrop-blur-[2px] md:items-center md:p-6"
          role="presentation"
          @click.self="closeInvoiceMenu"
        >
          <section
            class="max-h-[calc(100dvh-1rem)] w-full overflow-y-auto rounded-t-[1.75rem] border border-line bg-surface px-4 pb-[max(1.25rem,env(safe-area-inset-bottom))] pt-3 shadow-2xl md:max-w-lg md:rounded-2xl md:px-5 md:pb-6 md:pt-5"
            role="dialog"
            aria-modal="true"
      ...
      <section>
        ...
        <div class="grid grid-cols-2 gap-3" :class="{ 'animate-pulse': showSkeleton }">
          <a
            v-if="workshopEnabled"
            :href="`${base}/ordenes`"
      ...
      <section v-if="workshopEnabled || quotesPending" class="overflow-hidden rounded-2xl border border-line bg-surface shadow-sm">
        ...
        <div v-if="hasAttention" class="divide-y divide-line">
      ...
      <TaxEstimateCard :commercial="commercial" />

      <section v-if="workshopEnabled" class="overflow-hidden rounded-2xl border border-line bg-surface shadow-sm">
        <div class="flex items-center justify-between border-b border-line px-4 py-3">
          <h2 class="text-sm font-bold uppercase tracking-wide text-muted">Trabajo en curso</h2>
    ...
    <div class="hidden space-y-8 md:block" :aria-busy="loading">
    <section class="flex items-end justify-between gap-4 border-b border-line pb-5">
      <div>
        ...
        <h1 class="mt-1 text-[1.75rem] font-bold leading-tight text-text">{{ workshopEnabled ? 'Tu taller en un vistazo' : 'Tu facturación en un vistazo' }}</h1>
      </div>
    ...
    <section class="space-y-4">
      <h2 class="text-xs font-semibold uppercase tracking-wider text-soft">Hoy</h2>
      <div class="grid gap-5 lg:grid-cols-[minmax(0,1.3fr)_minmax(0,1fr)]">
        <UiCard class="flex flex-col justify-between gap-6">
          <div>
            <p class="text-sm font-medium text-muted">Ventas de hoy</p>
            <p class="mt-2 text-4xl font-bold tracking-tight text-text" :class="{ 'animate-pulse': showSkeleton }">{{ commercial ? money(commercial.sales_today) : '—' }}</p>
          ...
          <div class="flex flex-wrap items-center gap-2">
            ...
            <button type="button" class="inline-flex h-11 items-center gap-2 rounded-lg px-4 text-sm font-semibold transition" :class="workshopEnabled ? 'border border-line bg-surface text-text hover:bg-surface-muted' : 'bg-primary text-primary-contrast hover:bg-primary-hover'" aria-haspopup="dialog" @click="openInvoiceMenu"><FileCheck2 class="h-4 w-4" />Nueva factura<ChevronDown class="h-4 w-4 opacity-70" /></button>
          </div>
        </UiCard>
        ...
        <UiCard class="!p-0">
          ...
          <template v-else>
            ...
            <div class="flex items-center justify-between border-t border-line px-5 py-4"><span class="text-sm text-muted">IVA generado este mes</span><strong class="text-xl font-bold text-text">{{ commercial ? money(commercial.sales_tax_month) : '—' }}</strong></div>
          </template>
        </UiCard>
      </div>
    </section>

    <section v-if="hasAttention" class="space-y-4">
      <h2 class="text-xs font-semibold uppercase tracking-wider text-soft">Requieren atención</h2>
      <div class="overflow-hidden rounded-lg border border-line divide-y divide-line">
    ...
    <section class="space-y-4">
      <h2 class="text-xs font-semibold uppercase tracking-wider text-soft">Cumplimiento fiscal</h2>
      <div class="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(280px,0.8fr)]">
        <UiCard>
          <div class="flex items-center justify-between">
            <div><p class="font-semibold text-text">Estado fiscal</p><p class="mt-1 text-xs text-muted">Documentos procesados por Hacienda</p></div>
            <UiStatusBadge :tone="fiscalTone">{{ fiscalLabel }}</UiStatusBadge>
          </div>
          <div class="mt-5">
            <div class="flex items-baseline justify-between text-sm"><span class="font-semibold text-text">{{ fiscal?.totals.accepted ?? '—' }} aceptados</span><span :class="fiscal?.totals.rejected ? 'text-danger' : 'text-muted'">{{ fiscal?.totals.rejected ?? '—' }} rechazados</span></div>
            <div class="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-line"><div class="h-full rounded-full bg-success" :style="{ width: fiscalAcceptedPct + '%' }"></div></div>
          </div>
        </UiCard>
        <TaxEstimateCard :commercial="commercial" />
      </div>
    ...
    <section v-if="workshopEnabled" class="space-y-4">
      <h2 class="text-xs font-semibold uppercase tracking-wider text-soft">Actividad</h2>
